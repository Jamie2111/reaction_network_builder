...
export const PAGE = {
  ...
  synopsis: [
    ...
    'This operator representation is also the natural point of departure for tensor-network methods. The probability vector is represented as a [Matrix Product State](https://tensornetwork.org/mps/) and the generator as a [Matrix Product Operator](https://tensornetwork.org/mpo/), so that a state space that grows exponentially with the size of the system can be stored and propagated within a controllable memory budget[[c:4]]. The builder below assembles a reaction mechanism and displays its operator form and tensor diagrams; no quantum mechanics is needed to follow them.',
  ],
}
